...
def classify_minimal_enforcement(rows: List[Dict[str, Any]]) -> Tuple[bool, str, Dict[str, Any]]:
    ...
    if marker is None:
        if rejection_tick is not None:
            return False, "minimal enforcement requires duplicate_creation_path", {}
        return True, "minimal_enforcement_not_applicable_absent", {
            "duplicate_creation_path": False,
            "duplicate_candidate_identity_complete": False,
            "minimal_enforcement": False,
        }
    ...
    if not complete:
        ...
        return True, "minimal_enforcement_not_applicable_partial", result
    ...
    if not isinstance(signatures, list):
        return False, "minimal enforcement requires loop_signatures list", {}
    if loop_count is None:
        return False, "minimal enforcement requires loop_count", {}

    loop_count = int(loop_count)
    normalized_signatures = [str(item) for item in signatures]
    unique_signatures = set(normalized_signatures)
    result.update(
        {
            "minimal_enforcement": True,
    ...
    if loop_count != len(normalized_signatures):
        return False, "loop_count must match loop_signatures length on rejection tick", {}

    if len(unique_signatures) != len(normalized_signatures):
        return True, "minimal_enforcement_broken", result

    return True, "minimal_enforcement_preserved", result


def main() -> int:
    ap = argparse.ArgumentParser(description="Validate Stage 5 minimal exclusion enforcement")
    ...
    ap.add_argument(
        ...
        choices=[
            ...
            "minimal_enforcement_not_applicable_absent",
            "minimal_enforcement_not_applicable_partial",
            "minimal_enforcement_preserved",
            "minimal_enforcement_broken",
        ],
    )
    args = ap.parse_args()
    ...
    try:
        ...
        valid, classification, result = classify_minimal_enforcement(rows)
    except Exception as e:
        print("FAIL: exception while validating exclusion minimal enforcement")
        print(str(e))
        return 3
